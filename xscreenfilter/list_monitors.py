...
'''
Credits to
https://github.com/rr-/screeninfo/blob/master/screeninfo/enumerators/xrandr.py

After the bug that appeared in October 2021, the display
names changed weirdly into "1\x00\x00" or "".

This function reads the monitors without using Xlib directly.
'''
...
def load_library(name: str):
    ...
    assert path, "Could not load " + name
    return ctypes.cdll.LoadLibrary(path)  # type: ignore
...
def list_monitors():
    ...
    monitors: T.List[T.Tuple[int, str]] = []
    ...
    try:
        ...
        for i in range(screen_resources.contents.noutput):
            ...
            try:
                ...
                try:
                    crtc = int(output_info.contents.crtc)
                    name = str(output_info.contents.name.decode())
                    monitors.append((crtc, name))
                finally:
